...
import { type RouteFormData, newRoute } from "@/schemas/routeForm.schema";
...
function Payment() {
    ...
    const moveDate = () => {
        ...
        const dateObj = new Date(dateData["selected-date"]);
        const day = dateObj.getDay();
        const month = dateObj.getMonth();
        const year = dateObj.getFullYear();

        if (dateData.time == "AM") {
            return `${months[month]} ${day}, ${year} at 8 a.m. to 10 a.m.`;
        }
        return `${months[month]} ${day}, ${year} at PM`;
    };

    return (
        <div>
            <p className=" text-amber-500 font-bold py-4">
